...
import { Download, Info, Trash2, ChevronDown, Check, AlertTriangle, Target, Thermometer as ThermometerIcon, Smartphone as SmartphoneIcon, XCircle, FileText, FileSpreadsheet } from "lucide-react";
        ...
        if (isAlcotestType && deviceConfig && 'points' in deviceConfig) {
          ...
          verifiedPoints.forEach(point => {
             sheetData.push({
              ...
              'Изм. 1 (введено)': formatNumberWithComma(point.readingsRaw?.[0], 1),
              'Изм. 2 (введено)': formatNumberWithComma(point.readingsRaw?.[1], 1),
              'Изм. 3 (введено)': formatNumberWithComma(point.readingsRaw?.[2], 1),
          ...
          <TabsContent key={tabInfo.value} value={tabInfo.value}>
            {currentDeviceEntries.length > 0 ? (
              <Accordion type="multiple" className="w-full space-y-2">
                {currentDeviceEntries.sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map((entry) => {
                             ...
                             <div className="space-y-3">
                               ...
                               <Table className="text-xs">
                                <TableHeader>
                                    ...
                                    <TableHead>Изм.1 (вв.)</TableHead>
                                    <TableHead>Изм.2 (вв.)</TableHead>
                                    <TableHead>Изм.3 (вв.)</TableHead>
