...
import { QrCode, Save, AlertTriangle, ArrowRight, ArrowLeft, Info, Check, XCircle, Settings2, Loader2, AlertCircle, Thermometer, Smartphone, Edit3, Target, ChevronRight, PcCase, FileUp, ListFilter } from "lucide-react";
...
export function DataEntryForm({ selectedDevice, selectedInspector, onSaveEntry, dataEntries }: DataEntryFormProps) {
 ...
 useEffect(() => {
  ...
  const resetFormSpecifics = useCallback((preserveSerialNumber = false, preserveSubDeviceTypeAndSN = false) => {
    if (!preserveSerialNumber && !preserveSubDeviceTypeAndSN) setSerialNumber("");
  ...
  const handleScanQrCode = () => {
    const trimmedQrInput = qrInput.trim();
    if (trimmedQrInput) {
      setSerialNumber(trimmedQrInput);
      setIsQrDialogOpen(false);
      setQrInput("");
      toast({ title: "Серийный номер обновлен", description: `Серийный номер установлен: ${trimmedQrInput}` });

      if (isEffectivelyThermometer && thermoCurrentStep === 0) {
        handleThermoNextSNStep(trimmedQrInput);
      } else if (isEffectivelyAlcotest && alcoCurrentStep === 0) {
        handleAlcoNextSNStep(trimmedQrInput);
      } else if (!isEffectivelyThermometer && !isEffectivelyAlcotest) {
        const dialogShown = checkForExistingSerialAndPrompt(trimmedQrInput, selectedDevice, selectedSubDeviceType, undefined, true);
 ...
 const handleThermoNextSNStep = (currentSNValue?: string) => {
    const snToUse = currentSNValue || serialNumber;
    if (!snToUse.trim()) {
 ...
 const handleThermoSaveCurrentPoint = useCallback(() => {
  ...
  const handleSerialNumberKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
      ...
      if (isEffectivelyThermometer && thermoCurrentStep === 0) {
        if(isFastVerifyMode) {
          if (checkForExistingSerialAndPrompt(trimmedSN, selectedDevice, selectedSubDeviceType, undefined, true)) return;
        }
        handleThermoNextSNStep(trimmedSN);
      } else if (isEffectivelyAlcotest && alcoCurrentStep === 0) {
        handleAlcoNextSNStep(trimmedSN);
      } else if (!isEffectivelyThermometer && !isEffectivelyAlcotest) {
        const dialogShown = checkForExistingSerialAndPrompt(trimmedSN, selectedDevice, selectedSubDeviceType, undefined, true);
        if (dialogShown) return;
      }
    }
  };
  ...
  const handleSerialNumberBlur = () => {
    const trimmedSN = serialNumber.trim();
    if (trimmedSN && !isOverwriteDialogOpen &&
        ...
        if ((!isEffectivelyThermometer && !isEffectivelyAlcotest) || (isEffectivelyThermometer && isFastVerifyMode)) {
            checkForExistingSerialAndPrompt(trimmedSN, selectedDevice, selectedSubDeviceType, undefined, true);
        }
    }
  };
            ...
            <Input
                ...
                onChange={(e) => {
                    ...
                    if (overwriteConfirmation) {
                        if(overwriteConfirmation.sn.toLowerCase() !== newSN.trim().toLowerCase() ||
                           overwriteConfirmation.deviceType !== selectedDevice ||
                    ...
                    return (
                        <Card
                            ...
                            className={cn(
                                ...
                                (!serialNumber.trim() || formIsLoading) && "opacity-60 cursor-not-allowed"
                            )}
                            onClick={() => {
                                if (!serialNumber.trim() || formIsLoading) {
                                    toast({ title: "Сначала введите S/N", description: "Серийный номер необходим для выбора типа поверки.", variant: "destructive" });
                                    return;
                                }
                                ...
                                if (subDeviceConfig.id === 'thermometer') {
                                    handleThermoNextSNStep(serialNumber.trim());
                                } else if (subDeviceConfig.id === 'alcotest') {
                                    handleAlcoNextSNStep(serialNumber.trim());
                                }
                            }}
                        >
          ...
          {allConfigPoints.map(point => {
            ...
            return (
                <Card
                    ...
                    onClick={() => { if (!formIsLoading && !isOverwriteDialogOpen) handleThermoPointSelect(point);}}
                >
  ...
  const getPointLabelForDialog = () => {
    ...
    const effectiveDeviceType = subDeviceType || deviceType;
    const config = DEVICE_CONFIGS[effectiveDeviceType];
            ...
            { (selectedDevice && !INSPECTOR_DEVICE_IDS.includes(selectedDevice as DeviceId) || (INSPECTOR_DEVICE_IDS.includes(selectedDevice as DeviceId) && selectedSubDeviceType)) &&
              ...
              <>
                ...
                <div className="flex flex-col space-y-2 pt-2">
                  {isEffectivelyThermometer && thermoCurrentStep === 0 && !isThermoAllPointsDone && (
                    <Button type="button" onClick={() => handleThermoNextSNStep(serialNumber)} className="w-full" disabled={formIsLoading || !serialNumber.trim() || isOverwriteDialogOpen}>Далее <ArrowRight className="ml-2 h-4 w-4" /></Button>
                  )}
                  ...
                  {isEffectivelyAlcotest && alcoCurrentStep === 0 && (
                     <Button type="button" onClick={() => handleAlcoNextSNStep(serialNumber)} className="w-full" disabled={formIsLoading || !serialNumber.trim() || isOverwriteDialogOpen}>Далее <ArrowRight className="ml-2 h-4 w-4" /></Button>
                  )}
                  ...
                  {activeDeviceConfig && !isEffectivelyThermometer && !isEffectivelyAlcotest &&
                   (selectedDevice && !INSPECTOR_DEVICE_IDS.includes(selectedDevice as DeviceId) || (INSPECTOR_DEVICE_IDS.includes(selectedDevice as DeviceId) && selectedSubDeviceType && !isEffectivelyAlcotest && !isEffectivelyThermometer )) &&
                   'fields' in activeDeviceConfig && (activeDeviceConfig as StandardDeviceConfig).fields.length > 0 && (
                    <Button type="submit" className="w-full" disabled={formIsLoading || !serialNumber.trim() || Object.values(formValues).some(v => typeof v === 'string' && v.trim() === "") || isOverwriteDialogOpen}>
